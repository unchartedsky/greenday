...
cp -f "${SSH_USERHOME}/tmp/etc/99-warn-text" /etc/update-motd.d/99-warn-text
chown root:root /etc/update-motd.d/99-warn-text
chmod 755 /etc/update-motd.d/99-warn-text

cp -f "${SSH_USERHOME}/tmp/etc/issue" /etc/issue
chown root:root /etc/issue
chmod 644 /etc/issue

cp -f "${SSH_USERHOME}/tmp/etc/issue.net" /etc/issue.net
